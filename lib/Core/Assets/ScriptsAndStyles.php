...
class ScriptsAndStyles extends Runnable
{
	...
	{
		if(!($ns = $this->container->get('plugin_namespace'))) {
			if(preg_match_all('/\b(\w)/', $this->container->get('plugin_name'), $matches)) {
				$ns = implode('', $matches[1]);
				$this->container->set('plugin_namespace', $ns);
			}
		}

		return strtolower("{$ns}_{$name}");
	}
	...
	{
		if($file === 'theme')
			return $this->container->get('plugin_version');

		if(strpos($file, $this->container->get('plugin_url')) !== false) {
			$file = str_replace(
				$this->container->get('plugin_url'),
				$this->container->get('plugin_path'),
				$file
			);
		}

		if(empty($file) || !file_exists($file))
			$file = $this->container->get('plugin_path') . 'index.php';
	 ...
	 * @return $this
	 */
	public function script($handle, $source, $dependencies = [], $version = 'asset', $footer = true, callable $blocker = null)
	{
		if(empty($handle))
			throw new \RuntimeException('A valid `$handle` is required to register a new style');
